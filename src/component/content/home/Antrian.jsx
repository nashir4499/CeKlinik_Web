  ...
  const lanjut = () => {
    ...
    if (antrian.antrianBerjalan + 1 <= antrian.pasienTerdaftar) {
      ...
      if (antrian.pasien[antrian.antrianBerjalan] !== undefined) {
        ...
        fbAntrian
          .child("pasien")
          .child(antrian.pasien[antrian.antrianBerjalan].nomor)
          .update({
            statusBaru: "Selesai",
          })
          .catch((err) => alert(err));
  ...
  const lewat = () => {
    ...
    if (antrian.antrianBerjalan !== 0) {
      ...
      if (antrian.antrianBerjalan + 1 <= antrian.pasienTerdaftar) {
        if (
          window.confirm(
            `Anda yakin ingin melewati antrian pasien nomor ${antrian.antrianBerjalan} dengan nama ${pasien.nama}?`
          )
        ) {
          // Push Nitifikasi
          if (antrian.pasien[antrian.antrianBerjalan] !== undefined) {
          ...
          fbAntrian
            .child("pasienTerlewat")
            .child(antrian.antrianBerjalan)
            .set({
              nama: pasien.nama,
        ...
        antrian.antrianBerjalan + 1 > antrian.jumlahAntrian ||
        antrian.antrianBerjalan + 1 > antrian.pasienTerdaftar
      ) {
        if (
          window.confirm(
            `Anda yakin ingin melewati antrian pasien nomor ${antrian.antrianBerjalan} dengan nama ${pasien.nama}?`
      ...
      if (window.confirm("Anda yakin ingin menutup antrian pendaftaran?")) {
        dbRef
          ...
          .then((snap) => {
            ...
            let lapPasien = {
              pasien: data.pasien,
              pasienTerlewat: data.pasienTerlewat,
            };
            const newTitle =
              new Date().toISOString().substring(0, 10) + "-" + user.userId;
            ...
            const dataStore = {
              ...
              totalKunjungan:
                antrian.pasienTerlewat !== undefined
                  ? antrian.pasienTerdaftar -
                    Object.keys(antrian.pasienTerlewat).length
                  : antrian.pasienTerdaftar,
              pasienDarurat: antrian.pasien.filter(
                (x) => x.status === "Segerakan"
              ).length,
              pasienBatal:
                antrian.pasienTerlewat !== undefined
                  ? Object.keys(antrian.pasienTerlewat).length
              ...
              listPasienBatal:
                data.pasienTerlewat !== undefined ? data.pasienTerlewat : 0,
            };
            ...
            db.collection("laporans")
              .doc(klinik.id)
              .set(buatData, { merge: true })
              .catch((err) => alert(err));
            ...
            lapPasien.pasien.forEach((pasienNya) => {
              ...
              if (!pasienNya.pasienId) {
                if (pasienNya.id) {
                  db.collection("laporans")
                    .doc(klinik.id)
                    .collection("pasienTerdaftar")
                    .where("id", "==", pasienNya.id)
                    .get()
                    .then((querySnapshot) => {
                      ...
                      if (querySnapshot.empty === true) {
                        db.collection("laporans")
                          .doc(klinik.id)
                          .collection("pasienBelumTerdaftar")
                          .doc(pasienNya.noHp)
                          .set(pasienNya)
                          .catch((err) => alert(err));
                      }
      ...
      {loading ? (
          ...
          <div className="antrian__opsi">
            <div className="antrian__nomber">
              <div className="nomber__header">
                <h5>
                  Jumlah Antrian {antrian.pasienTerdaftar}/
                  {antrian.jumlahAntrian} <i className="fas fa-edit"></i>
                </h5>
              ...
              {antrian.pasien !== undefined &&
                (antrian.statusDarurat !== undefined || null || 0 ? (
                  <NomorAntrian
                    noAntrian={antrian.antrianBerjalan}
                    statusDarurat={true}
                    pasien={
                      antrian.antrianBerjalan !== 0
                        ? antrian.pasien[antrian.statusDarurat]
                        : { nama: "", alamat: "" }
                    }
                  />
                ) : (
                  <NomorAntrian
                    ...
                    pasien={
                      antrian.antrianBerjalan !== 0
                        ? antrian.pasien[antrian.antrianBerjalan]
                        : { nama: "", alamat: "" }
                    }
                  />
                ))}
        ...
        <div className="antrian__buka" onSubmit={bukaPendaftaran}>
          <div className="antrian__buka__content">
            <h5 style={{ color: "grey" }}>
              Silahkan masukkan jumlah antrian yang akan dibuka
            </h5>
